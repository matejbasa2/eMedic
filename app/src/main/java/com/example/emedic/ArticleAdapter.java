...
import com.example.emedic.group.GroupRecyclerAdapter;
...
public class ArticleAdapter extends GroupRecyclerAdapter<String, Article> {
    ...
    public ArticleAdapter(Context context) {
        ...
        map.put("Pacienti", create(0));
        map.put("Naročila", create(1));
        map.put("Opozorila", create(2));
        titles.add("Pacienti");
        titles.add("Naročila");
        titles.add("Opozorila");
    ...
    protected void onBindViewHolder(RecyclerView.ViewHolder holder, Article item, int position) {
        ...
        mLoader.asBitmap().load(item.getImgUrl())
    ...
    private static List<Article> create(int p) {
        ...
        if (p == 0) {
            list.add(create("Magda Osenar",
                    "Hud kašelj in obolelo grlo že 6 dni",
                    "http://cms-bucket.nosdn.127.net/catchpic/2/27/27e2ce7fd02e6c096e21b1689a8a3fe9.jpg?imageView&thumbnail=550x0"));//SLIKA OSEBE
            list.add(create("Filip Lukič",
                    "prebavne motnje in prekomerno napenjanje",
                    "http://cms-bucket.nosdn.127.net/catchpic/c/c8/c8b0685089258b82f3ca1997def78d8d.png?imageView&thumbnail=550x0"));
            list.add(create("Janez Novak",
                    "bolečina v sklepu (sklepam, da je voda v kolenu)",
